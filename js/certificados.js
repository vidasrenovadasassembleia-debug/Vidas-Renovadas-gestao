...
async function carregarDadosIniciais() {
  ...
  try {
    ...
    ESTADO_CERTIFICADOS.configuracoes =
      dados.configuracoes || {};

    ESTADO_CERTIFICADOS.arquivos =
      dados.arquivos || {};

    ESTADO_CERTIFICADOS.cargos =
      dados.cargos || [];
...
function mostrarResultadosMembros(
  membros,
  configuracao
) {
  const area =
    $(configuracao.areaResultados);
...
async function registrarEImprimir(
  ...
  if (
    !formulario.elements.idMembro.value.trim()
  ) {
    definirMensagem(
  ...
  try {
    ...
    ESTADO_CERTIFICADOS.certificadoAtual =
      dados;
...
function montarCertificado(dados) {
  ...
  const assinatura =
    arquivos.assinatura
      ? `
        <img
          src="${escAttr(arquivos.assinatura)}"
          alt="Assinatura"
        >
      `
      : "";
  ...
  const data =
    dataExtenso(dados.dataCerimonia);
  ...
  if (dados.tipo === "CONSAGRACAO") {
    ...
    texto = `
      Certificamos que
      <span class="cert-nome">
        ${esc(
          dados.nome ||
          "Nome do membro"
        )}
      </span>
      foi consagrado(a) ao santo ministério
      no cargo de
      <span class="cert-destaque">
        ${esc(
          dados.cargo ||
          "cargo ministerial"
        )}
      </span>,
      para servir ao Reino de Deus com
      fidelidade, zelo e dedicação.
    `;
  } else {
    titulo =
    ...
    texto = `
      ...
      foi batizado(a) nas águas,
      em testemunho público de sua fé
      em Jesus Cristo, conforme os
  ...
  return `
    <div class="cert-borda">
      ...
      <h2 class="cert-titulo">
        ${titulo}
      </h2>

      <div class="cert-subtitulo">
        ${subtitulo}
      </div>
...
function renderizarHistorico() {
  ...
  const itens =
    ESTADO_CERTIFICADOS.historico.filter(
      (item) => {
        return normalizar(
