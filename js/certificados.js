...
async function registrarEImprimir(evento,tipo){
  ...
  try{const r=await chamarApi({acao:"emitirCertificado",dados});dados.numero=r.certificado.numero;ESTADO_CERTIFICADOS.certificadoAtual=dados;atualizarPreview(tipo==="CONSAGRACAO"?"consagracao":"apresentacao",dados);definirMensagem(`${dados.numero} registrado com sucesso.`,"success");await recarregarHistorico();setTimeout(()=>window.print(),250);}
  catch(erro){definirMensagem(erro.message,"error");}
}

function atualizarTodasPreviews(){atualizarPreview("consagracao");atualizarPreview("apresentacao");}
...
function montarCertificado(d){
  ...
  if(d.tipo==="CONSAGRACAO"){
    ...
    titulo="Certificado de Apresentação";const artigo=d.sexo==="MENINA"?"a":"o";texto=`Certificamos que <span class="cert-nome">${esc(d.nome||"Nome da criança")}</span>, filh${artigo} de <span class="cert-destaque">${esc(d.nomePai||"—")}</span> e <span class="cert-destaque">${esc(d.nomeMae||"—")}</span>, foi apresentad${artigo} ao Senhor conforme a fé cristã e os princípios da Palavra de Deus.`;
  }
  return `<div class="cert-borda"><div class="cert-topo">${logo}<div class="cert-igreja">${esc(igreja)}</div></div><h2 class="cert-titulo">${titulo}</h2><div class="cert-subtitulo">${d.tipo==="CONSAGRACAO"?"Consagração Ministerial":"Apresentação de Criança"}</div><div class="cert-texto">${texto}<p>Realizado em <span class="cert-destaque">${esc(local||"local da cerimônia")}</span>, no dia <span class="cert-destaque">${esc(data||"data da cerimônia")}</span>.</p></div><div class="cert-rodape"><div class="cert-assinatura">${assinatura}<strong>${esc(pastor)}</strong><br>Pastor responsável</div></div><div class="cert-numero">Nº ${esc(d.numero||"prévia")}</div></div>`;
}
...
function renderizarHistorico(){
  ...
  itens.forEach(item=>{const tr=document.createElement("tr");tr.innerHTML=`<td><strong>${esc(item.numero)}</strong></td><td>${item.tipo==="CONSAGRACAO"?"Consagração":"Apresentação"}</td><td>${esc(item.nome)}</td><td>${esc(dataBr(item.dataCerimonia))}</td><td>${esc(item.emitidoPor)}</td><td><button class="botao-reimprimir" type="button">Reimprimir</button></td>`;$("button",tr).addEventListener("click",()=>reimprimir(item));corpo.appendChild(tr);});
}
function reimprimir(item){const tipo=item.tipo==="CONSAGRACAO"?"consagracao":"apresentacao";abrirAba(tipo);atualizarPreview(tipo,item);setTimeout(()=>window.print(),200);}
