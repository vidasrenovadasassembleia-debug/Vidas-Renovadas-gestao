...
,cargos:[],historico:[],certificadoAtual:null
}
;
...
window.location.href="../index.html";
  ...
  $("#pesquisaMembro").addEventListener("keydown",e=>{
if(e.key==="Enter"){
e.preventDefault();
pesquisarMembro();
  ...
  $("#formApresentacao").addEventListener("input",()=>atualizarPreview("apresentacao"));

  $("#formConsagracao").addEventListener("submit",e=>registrarEImprimir(e,"CONSAGRACAO"));

  $("#formApresentacao").addEventListener("submit",e=>registrarEImprimir(e,"APRESENTACAO"));
...
async function carregarDadosIniciais(){
  ...
  try{
    ...
    const [dados,historico]=await Promise.all([chamarApi({
...
function preencherPadroes(){
  ...
  [$("#formConsagracao"),$("#formApresentacao")].forEach(form=>{
...
acao:"pesquisarMembrosCertificado",termo
}
);
mostrarResultadosMembros(r.membros||[]);
definirMensagem(`${
(r.membros||[]).length
}
 resultado(s) encontrado(s).`,"success");
...
b.type="button";
b.className="resultado-membro";
b.innerHTML=`<span><strong>${
esc(m.nomeCompleto)
}
</strong><br>${
esc(m.congregacao||"Sem congregação")
}
</span><span>${
esc(m.numeroCarteirinha||m.id)
}
<br>${
esc(m.situacao)
}
</span>`;
b.addEventListener("click",()=>selecionarMembro(m));
area.appendChild(b);
...
function selecionarMembro(m){
  ...
  const f=$("#formConsagracao");
f.elements.idMembro.value=m.id;
f.elements.numeroCarteirinha.value=m.numeroCarteirinha;
f.elements.nome.value=m.nomeCompleto;
f.elements.congregacao.value=m.congregacao||f.elements.congregacao.value;
$("#resultadosMembros").hidden=true;
$("#pesquisaMembro").value=m.nomeCompleto;
atualizarPreview("consagracao");

}

function obterDadosFormulario(form,tipo){
...
if(!form.reportValidity())return;
  ...
  try{
const r=await chamarApi({
acao:"emitirCertificado",dados
}
);
dados.numero=r.certificado.numero;
ESTADO_CERTIFICADOS.certificadoAtual=dados;
atualizarPreview(tipo==="CONSAGRACAO"?"consagracao":"apresentacao",dados);
definirMensagem(`${
dados.numero
}
 registrado com sucesso.`,"success");
...
atualizarPreview("consagracao");
atualizarPreview("apresentacao");

}

function atualizarPreview(tipo,dadosForcados){

  const form=tipo==="consagracao"?$("#formConsagracao"):$("#formApresentacao");

  const dados=dadosForcados||obterDadosFormulario(form,tipo==="consagracao"?"CONSAGRACAO":"APRESENTACAO");

  const alvo=tipo==="consagracao"?$("#previewConsagracao"):$("#previewApresentacao");
...
function montarCertificado(d){
  ...
  const logo=a.logo?`<img class="cert-logo" src="${
escAttr(a.logo)
}
" alt="Logo">`:"";

  const assinatura=a.assinatura?`<img src="${
escAttr(a.assinatura)
}
" alt="Assinatura">`:"";
  ...
  if(d.tipo==="CONSAGRACAO"){
    ...
    titulo="Certificado";
texto=`Certificamos que <span class="cert-nome">${
esc(d.nome||"Nome do membro")
}
</span> foi consagrado(a) ao santo ministério no cargo de <span class="cert-destaque">${
esc(d.cargo||"cargo ministerial")
}
</span>, para servir ao Reino de Deus com fidelidade, zelo e dedicação.`;
...
const artigo=d.sexo==="MENINA"?"a":"o";
texto=`Certificamos que <span class="cert-nome">${
esc(d.nome||"Nome da criança")
}
</span>, filh${
artigo
}
 de <span class="cert-destaque">${
esc(d.nomePai||"—")
}
</span> e <span class="cert-destaque">${
esc(d.nomeMae||"—")
}
</span>, foi apresentad${
artigo
}
 ao Senhor conforme a fé cristã e os princípios da Palavra de Deus.`;

  
}

  return `<div class="cert-borda"><div class="cert-topo">${
logo
}
<div class="cert-igreja">${
esc(igreja)
}
</div></div><h2 class="cert-titulo">${
titulo
}
</h2><div class="cert-subtitulo">${
d.tipo==="CONSAGRACAO"?"Consagração Ministerial":"Apresentação de Criança"
}
</div><div class="cert-texto">${
texto
}
<p>Realizado em <span class="cert-destaque">${
esc(local||"local da cerimônia")
}
</span>, no dia <span class="cert-destaque">${
esc(data||"data da cerimônia")
}
</span>.</p></div><div class="cert-rodape"><div class="cert-assinatura">${
assinatura
}
<strong>${
esc(pastor)
}
</strong><br>Pastor responsável</div></div><div class="cert-numero">Nº ${
esc(d.numero||"prévia")
}
</div></div>`;
...
ESTADO_CERTIFICADOS.historico=r.certificados||[];
renderizarHistorico();
...
const itens=ESTADO_CERTIFICADOS.historico.filter(i=>normalizar(`${
i.numero
}
 ${
i.nome
...
item.tipo==="CONSAGRACAO"?"Consagração":"Apresentação"
}
</td><td>${
esc(item.nome)
}
</td><td>${
esc(dataBr(item.dataCerimonia))
}
</td><td>${
esc(item.emitidoPor)
}
</td><td><button class="botao-reimprimir" type="button">Reimprimir</button></td>`;
$("button",tr).addEventListener("click",()=>reimprimir(item));
...
const tipo=item.tipo==="CONSAGRACAO"?"consagracao":"apresentacao";
abrirAba(tipo);
